...
from .tools.function import init_embedding
from .models import Encoder, Decoder, Discriminator, Generator
from .tools.utils import denorm_image, centering_image
...
class Trainer:
    ...
    def __init__(self, GPU, data_dir, fixed_dir, fonts_num, batch_size, img_size):
        ...
        with open(os.path.join(fixed_dir, 'EMBEDDINGS.pkl'), 'rb') as f:
            self.embeddings = pickle.load(f)
        
        self.embedding_num = self.embeddings.shape[0]
        self.embedding_dim = self.embeddings.shape[3]
        
        self.fixed_source = torch.load(os.path.join(fixed_dir, 'fixed_source.pkl'), map_location=torch.device('cuda'))
        self.fixed_target = torch.load(os.path.join(fixed_dir, 'fixed_target.pkl'), map_location=torch.device('cuda'))
        self.fixed_label = torch.load(os.path.join(fixed_dir, 'fixed_label.pkl'), map_location=torch.device('cuda'))
    ...
    def train(self, max_epoch, schedule, save_path, to_model_path, lr=0.001, \
        ...
        if len(restore) == 3:
            ...
            En.load_state_dict(torch.load(os.path.join(from_model_path, encoder_path), map_location=torch.device('cuda')))
            De.load_state_dict(torch.load(os.path.join(from_model_path, decoder_path), map_location=torch.device('cuda')))
            D.load_state_dict(torch.load(os.path.join(from_model_path, discriminator_path), map_location=torch.device('cuda')))
            ...
            En.load_state_dict(torch.load(os.path.join(from_model_path, encoder_path), map_location=torch.device('cuda')))
            De.load_state_dict(torch.load(os.path.join(from_model_path, decoder_path), map_location=torch.device('cuda')))
        # restore에 [encoder_path] 넣기 (fine-tunning)
        elif len(restore) == 1:
            encoder_path = restore[0]
            prev_epoch = 0
            En.load_state_dict(torch.load(os.path.join(from_model_path, encoder_path), map_location=torch.device('cuda')))
        else:
            prev_epoch = 0
