...
import os
import torch
from .tools.utils import pad_seq, bytes_to_file, read_split_image, round_function
from .tools.utils import shift_and_resize_image, normalize_image, centering_image
    ...
    def process(img):
        ...
        try:
            ...
            if augment:
                ...
                # to be shifted
                w, h = img_A.shape
                multiplier = random.uniform(1.00, 1.1)
...
def save_fixed_sample(sample_size, img_size, data_dir, save_dir, \
    ...
    for batch in train_batch_iter:
        ...
        fixed_source = torch.tensor(np.array([i[1] for i in source_with_label]))
        fixed_target = torch.tensor(np.array([i[1] for i in target_with_label]))
